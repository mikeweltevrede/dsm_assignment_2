...
\begin{align*}
    E[\hat{\beta}(\lambda)|X] &= E[(X'X + \lambda I_{p})^{-1}X'y|X]\\
                              &=(X'X + \lambda I_{p})^{-1}X'E[(X\beta + \epsilon)|X]\\
                              &= (X'X + \lambda I_{p})^{-1}X'X\beta +(X'X + \lambda I_{p})^{-1}X' E[\epsilon|X]\\
                              &= (X'X + \lambda I_{p})^{-1}X'X\beta + (X'X + \lambda I_{p})^{-1}X' \times 0\\
                              &= (X'X + \lambda I_{p})^{-1}X'X\beta\\
                              &= (X'X + \lambda I_{p})^{-1}(X'X + \lambda I_{p} - \lambda I_{p})\beta\\
                              &= (I_{p} - \lambda(X'X + \lambda I_{p})^{-1})\beta\\
                              &= \beta - \lambda(X'X + \lambda I_{p})^{-1}\beta\\
                              \\
    E[\hat{\beta}(\lambda)|X] - \beta &= \beta - \lambda(X'X + \lambda I_{p})^{-1}\beta - \beta\\
                                      &= -\lambda(X'X + \lambda I_{p})^{-1}\beta\\
                                      &= -\lambda W(\lambda)\beta.
\end{align*}

### Subquestion b: Efficiency of the ridge estimator
From the previous exercise, we have that:
\begin{align*}
    \hat{\beta_\lambda}          &= (X'X + \lambda I_{p})^{-1}X'X\hat{\beta}\\
    Var[\hat{\beta_{\lambda}}|X] &= (X'X + \lambda I_{p})^{-1}X'X Var[\hat{\beta}|X][(X'X + \lambda I_{p})X'X]'\\
                                 &= (X'X + \lambda I_{p})^{-1}X'X Var[\hat{\beta}|X]X'X(X'X + \lambda I_{p})^{-1}\\
                                 &= (X'X + \lambda I_{p})^{-1}X'X \sigma^{2}(X'X)^{-1}X'X(X'X + \lambda I_{p})^{-1}\\
                                 &= (X'X + \lambda I_{p})^{-1} (\sigma^{2}X'X)(X'X + \lambda I_{p})^{-1}\\
                                 &= W(\lambda)(\sigma^{2}X'X)W(\lambda)
\end{align*}

When $\lambda = 0$ we are left with the OLS estimator. So for $Var(\hat{\beta}(0)|X)-Var(\hat{\beta}(\lambda)|X)$ to be positive definite we need $Var(\hat{\beta}(\lambda)|X)<Var(\hat{\beta}(0)|X)$.

\begin{align*}
    (X'X)^{-1} - WX'XW &= WW^{-1}(X'X)^{-1}W^{-1}W - WX'XW\\
    &= W(W^{-1}(X'X)^{-1}W^{-1}-X'X)W\\
\end{align*}

Now we will simplify the inside of the brackets.

\begin{align*}
    W^{-1}(X'X)^{-1}W^{-1}-X'X&= (X'X+\lambda I)(X'X)^{-1}(X'X \lambda I) - X'X\\
    &= X'X(X'X)^{-1}X'X + X'X(X'X)^{-1}\lambda + \lambda(X'X)^{-1}X'X + \lambda^{2}(X'X)^{-1}-X'X\\
    &= 2\lambda I + \lambda^{2} (X'X)^{-1}\\
    &= \lambda (2I + \lambda (X'X)^{-1})
\end{align*}

Since $(X'X)^{-1}$ is invertible it is positive definite. Therefore the whole expression is positive definite. Then we can put the this back into the original expression to get:

\[\lambda W(2I + \lambda (X'X)^{-1})W\]

Which is positive definite, as the expression inside the brackets is also positive definite. Which shows that $Var(\hat{\beta}(0)|X)-Var(\hat{\beta}(\lambda)|X)$ is positive definite, making the ridge estimator more efficient than OLS estimator.
